...
WifiClock::WifiClock(Display &display, FS &fs)
    : display(display), timeClient(ntpUDP), timezone(rule), fs(fs) {
  _ntpEnabled = false;
}
...
void WifiClock::begin() {
  ...
  WiFi.onEvent([](arduino_event_t *event) { clockInstance->sync(); },
               ARDUINO_EVENT_WIFI_STA_GOT_IP);

  displayTime.attach(.2, [] { clockInstance->updateDisplay(); });
...
void WifiClock::updateDisplay() {
  ...
  if (timeElements.Second != lastSecond) {
    ...
    auto time = timeElements.Hour * 10000 + timeElements.Minute * 100 +
                timeElements.Second;
...
void WifiClock::loadConfig(bool first) {
  ...
  if (_ntpEnabled) {
    ...
    updateTime.attach(updateInterval, [] { clockInstance->sync(); });
