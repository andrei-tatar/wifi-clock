...
void Display::invalidateDigitCache() { memset(lastDigits, 0xFF, 6); }
...
void Display::drawTime(uint32_t time) {
  ...
  for (uint8_t digit = 0; digit < 10; digit++) {
    ...
    if (updateIndex ==
            MAX_UPDATES || // if we can't hold any more updates in memory
        (digit == 9 && updateIndex)) // or we got to the last digit and we have
                                     // pending updates
    {
...
#define OUT_BUF_SIZE 32768
  ...
  while (remaining_compressed > 0 && status > TINFL_STATUS_DONE) {
    ...
    status = tinfl_decompress(&inflator, data_buf, &in_bytes, out_buf, next_out,
                              &out_bytes, flags);
